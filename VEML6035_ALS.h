#ifndef _VEML6035_ALS_H_
#define _VEML6035_ALS_H_
...
const uint8_t defAddr = 0x29; // Default I2C address

enum VEML6035_16BIT_REGISTERS {
...
enum VEML6035_16BIT_REG_MASKS {
  ...
  INT_MASK               = 0xC000,
  WHITE_CH_EN_MASK       = 0xFFFB,
  INT_CHANNEL_MASK       = 0xFFF7,
  DG_MASK                = 0xF7FF,
  SENS_MASK              = 0xEFFF
};
...
enum REGISTER_BIT_POSITIONS {
  ...
  DG_POS                 = 0xB,
  SENS_POS               = 0xC,
  INT_CHANNEL_POS        = 0x03,
  WHITE_CH_EN_POS        = 0x02

};

// Table of lux conversion values depending on the integration time, sense, DG and gain. 
// The arrays are copied from the datasheet page 9: 
// https://www.vishay.com/docs/84889/veml6035.pdf

const float eightHItDG[]     = {.0004, .0008, .0032, .0064};
const float fourHItDG[]      = {.0008, .0016, .0064, .0128};
const float twoHItDG[]       = {.0016, .0032, .0128, .0256};
const float oneHItDG[]       = {.0032, .0064, .0256, .0512};
const float fiftyItDG[]      = {.0064, .0128, .0512, .1024};
const float twentyFiveItDG[] = {.0128, .0256, .1024, .2048};

const float eightHItnoDG[]     = {.0008, .0016, .0064, .0128};
const float fourHItnoDG[]      = {.0016, .0032, .0128, .0256};
const float twoHItnoDG[]       = {.0032, .0064, .0256, .0512};
const float oneHItnoDG[]       = {.0064, .0128, .0512, .1024};
const float fiftyItnoDG[]      = {.0128, .0256, .1024, .2048};
const float twentyFiveItnoDG[] = {.0256, .0512, .2048, .4096};

class VEML6035
{  
  public:

    VEML6035(uint8_t address); // I2C Constructor
    ...
    void setIntChannel(bool channelVal);
    ...
    void setIntLowThreshRAW(uint16_t rawVal);
    ...
    // This function sets the upper limit for the Ambient Light Sensor's interrupt. 
    // It takes the raw value as its paramater.
    void setIntHighThreshRAW(uint16_t rawVal);
    ...
    // REG[0x04], bits[15:0]
    // This function gets the sensor's ambient light's RAW value.
